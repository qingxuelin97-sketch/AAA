...
function avatar(name, c1, c2, emoji) {
  ...
  <rect width="400" height="400" fill="url(#g)"/><circle cx="300" cy="110" r="140" fill="#fff" opacity="0.08"/>
  <circle cx="90" cy="320" r="100" fill="#000" opacity="0.12"/><text x="200" y="252" font-size="150" text-anchor="middle">${emoji}</text></svg>`);
}
...
const u1 = user('demo', '旅人', '热爱奇幻与角色扮演的创作者，正在书写属于自己的幻域。', avatar('u_demo.svg', '#a779ff', '#3a2566', '🧳'), banner('bn_demo.svg', '#3a2566', '#15102e'), 18600, 320, 30);
const u2 = user('astra', '星语者', '专注科幻与赛博朋克题材的世界观构筑师。', avatar('u_astra.svg', '#37d6e0', '#103040', '🛰️'), banner('bn_astra.svg', '#103040', '#0a1622'), 9200, 60, 0);
const u3 = user('mochi', '麻薯', '治愈系日常向作者，喜欢一切软软的东西。', avatar('u_mochi.svg', '#ff9ec4', '#6e2f4d', '🍡'), banner('bn_mochi.svg', '#6e2f4d', '#2a1620'), 4300, 0, 0);
const u4 = user('kenji', '剑持', '武侠与历史题材，刀光剑影里见人心。', avatar('u_kenji.svg', '#d8a657', '#5a3d1f', '🗡️'), banner('bn_kenji.svg', '#5a3d1f', '#221409'), 6700, 10, 0);
...
const cVeil = char(u1, { name: '森灵 · 薇尔', category: 'fantasy', tags: '奇幻,精灵,治愈', uses: 1240, likes: 356,
  avatar: avatar('a_veil.svg', '#3fae7d', '#15402f', '🧝‍♀️'), background: bg('bg_forest.svg', '#1d4d39', '#0c2018', '#0a3322', 'forest'),
...
const cK = char(u2, { name: '赛博侦探 · K', category: 'scifi', tags: '科幻,赛博朋克,悬疑', uses: 980, likes: 412,
  avatar: avatar('a_k.svg', '#37d6e0', '#10303a', '🕵️'), background: bg('bg_cyber.svg', '#0e2a3a', '#1a0f2e', '#ff4f9d', 'soft'),
...
const cMian = char(u3, { name: '猫娘咖啡店长 · 棉花', category: 'daily', tags: '日常,治愈,猫娘', uses: 2130, likes: 880,
  avatar: avatar('a_mochi.svg', '#ff9ec4', '#6e2f4d', '🐱'), background: bg('bg_cafe.svg', '#7a4a5e', '#2a1620', '#ffd5a8', 'soft'),
...
const cYun = char(u4, { name: '剑客 · 云无意', category: 'wuxia', tags: '武侠,江湖,侠义', uses: 760, likes: 233,
  avatar: avatar('a_yun.svg', '#c0c8d8', '#2a3340', '🥷'), background: bg('bg_wuxia.svg', '#2a3340', '#10151c', '#7a8aa0', 'soft'),
...
const cLuna = char(u2, { name: '星舰 AI · 露娜', category: 'scifi', tags: '科幻,太空,AI', uses: 540, likes: 190,
  avatar: avatar('a_luna.svg', '#7aa7ff', '#1a2350', '🌐'), background: bg('bg_star.svg', '#241a4a', '#0c0b20', '#fff', 'stars'),
...
cmt.run(m1, u1, '棉花太可爱了！已收藏 ❤️'); cmt.run(m1, u2, '咖啡店背景图绝了');
...
const g1 = group(u1, '幻域创作者联盟', '角色卡 / 剧本创作交流，互相催更 📝', avatar('g_create.svg', '#a779ff', '#3a2566', '🎨'), [u2, u3, u4]);
group(u2, '赛博朋克爱好者', '霓虹、义体与雨夜，欢迎同好。', avatar('g_cyber.svg', '#37d6e0', '#103040', '🌃'), [u1]);
group(u3, '治愈系小窝', '分享一切温柔软糯的角色与日常 🍮', avatar('g_heal.svg', '#ff9ec4', '#6e2f4d', '🌸'), [u1, u4]);
