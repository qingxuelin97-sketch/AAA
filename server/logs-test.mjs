...
// 规则告警引擎 / 实时日志流（SSE live tail）/ 查询书签 / 归档 / 延迟分位 /
// 热力图 / 会话轨迹 / 健康自检 / 导出格式 / 定向清理 / 审计留痕。
// 跑法：npm run test:logs
//
...
try {
  ...
  const clientLog = (body) => fetch(BASE + '/logs/client', {
  ...
  const batchRes = await (await clientLog({
    batch: [
  ...
  await clientLog({ level: 'fatal', event: 'logstest_poison', message: '投毒样本' });
  ...
  await clientLog({ level: 'bogus', event: 'logstest_bogus_level', message: '非法级别' });
  ...
  await clientLog({ level: 'warn', event: 'logstest_count', message: '重复汇总样本', count: 5 });
  ...
  assert.ok(stats.prev_24h && typeof stats.prev_24h.requests === 'number', '统计应含昨日同窗对比');
  ...
  const latency = await j('/admin/logs/latency');
  assert.ok(typeof latency.global?.p50 === 'number' && typeof latency.global?.p95 === 'number' && typeof latency.global?.p99 === 'number', '延迟分位应含 P50/P95/P99');
  assert.ok(Array.isArray(latency.endpoints), '延迟分位应含逐接口明细');
  const { matrix } = await j('/admin/logs/heatmap');
  assert.equal(matrix.length, 7, '热力图必须 7 行（星期）');
  assert.ok(matrix.every(row => row.length === 24), '热力图每行必须 24 列（小时）');
  const { sessions } = await j('/admin/logs/sessions');
  assert.ok(sessions.some(s => s.session_id === 'sess-logstest-1'), '会话轨迹应聚合出测试会话');
  const testSess = sessions.find(s => s.session_id === 'sess-logstest-1');
  assert.ok(testSess.n >= 3 && testSess.errors >= 1, `会话聚合应带条数与错误数（got n=${testSess.n} errors=${testSess.errors}）`);
  console.log('  ✅ 统计/时序/延迟分位/热力图/会话轨迹');

  /* ── 6. 保留策略：GM 可调 + 1–365 钳制 + 归档开关 ── */
  const ret = await j('/admin/logs/retention', { method: 'PUT', body: JSON.stringify({ debug: 9999, info: 0, archive: true }) });
  assert.equal(ret.retention.debug, 365, '保留天数上限钳制 365');
  assert.equal(ret.retention.info, 1, '保留天数下限钳制 1');
  assert.equal(ret.retention.archive, true, '归档开关应保存');
  await j('/admin/logs/retention', { method: 'PUT', body: JSON.stringify({ debug: 3, info: 7 }) });
  console.log('  ✅ 保留策略：可配置、1–365 钳制、归档开关');

  /* ── 7. 规则告警引擎：错误风暴 + 指定事件监控 ── */
  const putRules = await j('/admin/logs/alerts', {
    method: 'PUT',
    body: JSON.stringify({
      rules: [
        { name: '测试错误风暴', enabled: true, type: 'error_burst', threshold: 3, window_min: 5, cooldown_min: 15 },
        { name: '监控-测试事件', enabled: true, type: 'event_match', event: 'logstest_watch', threshold: 2, window_min: 5, cooldown_min: 15 },
      ],
    }),
  });
  assert.equal(putRules.rules.length, 2, '规则应保存 2 条');
  assert.ok(putRules.rules.every(r => r.id), '每条规则应分配 id');
  // 错误风暴：3 条错误触发
  await clientLog({
    batch: [
  ...
  await sleep(300);
  const stormNote = db.prepare("SELECT * FROM notifications WHERE user_id = ? AND text LIKE '%测试错误风暴%' ORDER BY id DESC LIMIT 1").get(uid);
  assert.ok(stormNote, '错误达到阈值后 GM 必须收到站内告警通知（含规则名）');
  // 事件监控：指定事件 2 次触发
  await clientLog({
    batch: [
      { level: 'warn', event: 'logstest_watch', message: '监控事件一' },
      { level: 'warn', event: 'logstest_watch', message: '监控事件二' },
    ],
  });
  await sleep(300);
  const watchNote = db.prepare("SELECT * FROM notifications WHERE user_id = ? AND text LIKE '%监控-测试事件%' ORDER BY id DESC LIMIT 1").get(uid);
  assert.ok(watchNote, '事件监控规则达到阈值后 GM 必须收到通知');
  const fired = await j('/admin/logs?' + new URLSearchParams({ event: 'log_alert_fired' }));
  assert.ok(fired.total >= 2, '每次告警触发都要留痕（log_alert_fired）');
  console.log('  ✅ 规则告警引擎：错误风暴 + 事件监控均触发通知与留痕');

  /* ── 8. 实时日志流（live tail）：SSE 端到端 ── */
  const { ticket } = await j('/realtime/ticket', { method: 'POST' });
  sseAbort = new AbortController();
  const sseRes = await fetch(BASE + '/realtime/stream?ticket=' + ticket, { signal: sseAbort.signal });
  assert.equal(sseRes.status, 200, 'SSE 流应可用票据建立');
  const reader = sseRes.body.getReader();
  await j('/admin/logs/tail', { method: 'POST', body: JSON.stringify({ enabled: true }) });
  await clientLog({ level: 'info', event: 'logstest_tail_probe', message: 'tail 探针' });
  const sawTail = await (async () => {
    const dec = new TextDecoder();
    let buf = '';
    const deadline = Date.now() + 8000;
    while (Date.now() < deadline) {
      const r = await Promise.race([reader.read(), sleep(deadline - Date.now()).then(() => ({ done: true }))]);
      if (r.done) break;
      buf += dec.decode(r.value, { stream: true });
      if (buf.includes('event: logline') && buf.includes('logstest_tail_probe')) return true;
    }
    return false;
  })();
  assert.ok(sawTail, 'tail 开启后新日志必须以 logline 事件推送到 GM 的 SSE');
  await j('/admin/logs/tail', { method: 'POST', body: JSON.stringify({ enabled: false }) });
  sseAbort.abort(); sseAbort = null;
  console.log('  ✅ 实时日志流：SSE logline 秒级到达');

  /* ── 9. 查询书签 ── */
  const putViews = await j('/admin/logs/views', {
    method: 'PUT',
    body: JSON.stringify({ views: [{ name: '错误·近24h', params: { level: 'error', preset: '24h' } }] }),
  });
  assert.equal(putViews.views.length, 1, '书签应保存');
  assert.ok(putViews.views[0].id, '书签应分配 id');
  const gotViews = await j('/admin/logs/views');
  assert.equal(gotViews.views[0].name, '错误·近24h', '书签读回一致');
  assert.equal(gotViews.views[0].params.level, 'error', '书签参数读回一致');
  console.log('  ✅ 查询书签：保存/读回');

  /* ── 10. 归档：立即归档 / 列表 / 下载（gunzip 校验）/ 删除 ── */
  const arch = await j('/admin/logs/archives', { method: 'POST', body: JSON.stringify({ days: 0 }) });
  assert.ok(arch.count > 0 && arch.file, `立即归档应产出文件（got ${JSON.stringify(arch)}）`);
  const archList = await j('/admin/logs/archives');
  assert.ok(archList.archives.some(a => a.file === arch.file), '归档列表应包含新文件');
  const dl = await fetch(BASE + '/admin/logs/archives/' + encodeURIComponent(arch.file), { headers: H });
  assert.equal(dl.status, 200, '归档下载应 200');
  const gunzipped = zlib.gunzipSync(Buffer.from(await dl.arrayBuffer())).toString('utf8');
  const archLines = gunzipped.trim().split('\n');
  assert.equal(archLines.length, arch.count, '归档行数应与 count 一致');
  assert.ok(archLines.every(l => { JSON.parse(l); return true; }), '归档每行都是合法 JSON');
  await j('/admin/logs/archives/' + encodeURIComponent(arch.file), { method: 'DELETE' });
  const archList2 = await j('/admin/logs/archives');
  assert.ok(!archList2.archives.some(a => a.file === arch.file), '删除后归档列表不应再包含该文件');
  console.log('  ✅ 归档：立即归档 → 下载 gunzip 校验 → 删除');

  /* ── 11. 导出：CSV / NDJSON 格式与审计留痕 ── */
  ...
  const purgeArch = await j('/admin/logs/archives');
  assert.ok(purgeArch.archives.some(a => a.file.startsWith('logs-purge-')), '开启归档后清理前应自动存档');
  console.log('  ✅ 定向清理：level+days 生效，清理前自动归档');

  /* ── 13. 健康自检 + 配置审计 ── */
  const { health } = await j('/admin/logs/health');
  assert.ok(health.rows >= 1 && health.retention && typeof health.alert_rules === 'number', '健康自检应含行数/保留策略/规则数');
  assert.ok(typeof health.dedup_ratio === 'number' && health.ingest_last_hour >= 1, '健康自检应含去重比与摄入速率');
  ...
  const auditArch = await j('/admin/logs?' + new URLSearchParams({ category: 'admin', event: 'logs_archive' }));
  assert.ok(auditArch.total >= 1, '手动归档必须进审计');
  console.log('  ✅ 健康自检 + 配置/归档审计留痕');
  ...
  srv.kill();
  for (const f of [DB_PATH, DB_PATH + '-wal', DB_PATH + '-shm']) { try { fs.unlinkSync(f); } catch { /* */ } }
  try { fs.rmSync(path.join(__dirname, 'log-archives'), { recursive: true, force: true }); } catch { /* */ }
}
process.exit(ok ? 0 : 1);
