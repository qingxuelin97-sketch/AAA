// 个人主页（/profile 自己 · /user/:id 他人）—— Lumen Glass 大更新（任务 D）。
// · Web（!appMode）：Lumen 化 banner/头像区（.pfw-* + .lgw-card 三件套，零实时
//   模糊）；仅自己的主页叠加 MembershipBanner（会员横幅）+ AssetCard（资产卡）；
//   内容区换 ProfileContentTabs（角色/剧本/动态三 Tab 保留，加骨架与
//   ProfileLoadError 错误重试）。好友四态/关注/私信/举报/粉丝弹层/CountUp 等
//   社交能力全部原样保留；他人主页不渲染会员/资产模块。
// · App（appMode）：渲染分支与改前逐字节一致 —— App 模式零波及铁律。
...
import { Settings, ScrollText, UserPlus, UserCheck, LogOut, Wallet, Heart, BadgeCheck, Crown, X, Pencil, Share2, Check, MessageSquare } from 'lucide-react';
...
import '../styles/web-lumen-profile.css';
...
export default function Profile() {
  ...
  const appMode = isAppMode(); // App 壳专用视觉钩子；App DOM 保持零差异
  const targetId = id || user?.id;
  const isMe = String(targetId) === String(user?.id);
  const [data, setData] = useState(null);
  const [loadError, setLoadError] = useState(''); // Web 端载入失败态（App 分支不消费，行为与改前一致）
  ...
  const load = () => {
    setLoadError('');
    return api('/users/' + targetId)
      .then(d => { setData(d); setFollowing(d.following); })
      .catch(e => { setLoadError(e.message || '主页暂时无法载入'); toast(e.message, 'err'); });
  };
  ...
  // 粉丝 / 关注弹层 —— 双分支共用同一份 DOM（App 侧与改前逐字节一致）
  const listModalJsx = listModal && (
  ...
  if (appMode) {
    // ═══════════════════════════════════════════════════════════════════
    // App 壳分支：以下渲染与 Lumen 更新前逐字节一致（qa-* 钩子原样）。
    // ═══════════════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════════════
  // Web · Lumen Glass 分支（.pfw-* / web-lumen-profile.css）
  // ═══════════════════════════════════════════════════════════════════
  const webTabs = [
    { key: 'characters', label: '角色', count: data ? data.characters.length : undefined },
    { key: 'scripts', label: '剧本', count: data ? data.scripts.length : undefined },
    { key: 'moments', label: '动态', count: data ? data.moments.length : undefined }
  ];
  ...
  return (
    <>
      <div className="topbar">
        <div style={{ flex: 1 }}>
          <h1>{isMe ? '个人中心' : (u ? u.display_name : '个人主页')}</h1>
          {u && <div className="sub">@{u.username} · {pid('user', u.id)}</div>}
        </div>
        {u && <button className="btn ghost" onClick={shareProfile} title="复制主页链接" aria-label="复制主页链接"><Share2 size={15} /></button>}
        {u && (isMe ? (
          <>
            <button className="btn" onClick={() => nav('/settings')}><Settings size={15} /> 设置</button>
            <button className="btn ghost" onClick={logout} title="退出登录" aria-label="退出登录"><LogOut size={15} /></button>
          </>
        ) : (
          <>
            <button className={'btn ' + (fr.state === 'friends' ? '' : 'primary')} onClick={friendAction}>
      ...
      <div className="page pfw">
        {/* banner / 头像区（Lumen 玻璃卡：半透明底 + 三件套阴影，零实时模糊） */}
        <section className={'pfw-hero lgw-card' + (!data && loadError ? ' err' : '')}>
          {u ? (
            <>
              ...
              <div className="pfw-head">
                ...
                <div className="pfw-head-main">
                  ...
                  <p className="pfw-bio">{u.bio || '这位玩家还没有写简介'}</p>
                </div>
                {isMe && (
                  <button type="button" className="pfw-edit" onClick={() => nav('/settings')}><Pencil size={14} /> 编辑资料</button>
                )}
              </div>
              <div className="pfw-stats">
                <button type="button" className="pfw-stat" onClick={() => setTab('characters')}><b><CountUp value={data.stats.characters} /></b><span>角色</span></button>
                <button type="button" className="pfw-stat" onClick={() => setTab('scripts')}><b><CountUp value={data.stats.scripts} /></b><span>剧本</span></button>
                <button type="button" className="pfw-stat" onClick={() => openList('followers')}><b><CountUp value={data.stats.followers} /></b><span>粉丝</span></button>
                <button type="button" className="pfw-stat" onClick={() => openList('following')}><b><CountUp value={data.stats.following} /></b><span>关注</span></button>
                {data.stats.achievements != null && (
                  <button type="button" className={'pfw-stat' + (isMe ? '' : ' inert')} onClick={() => isMe && nav('/achievements')}>
                    <b><CountUp value={data.stats.achievements} /></b><span>成就</span>
                  </button>
                )}
              </div>
            </>
            ...
            <>
              {/* 骨架（载入失败时由 .err 静止） */}
              <div className="pfw-banner pfw-skel" aria-hidden="true" />
              <div className="pfw-head" aria-hidden="true">
                <span className="pfw-avatar pfw-skel" />
                <div className="pfw-head-main">
                  <span className="pfw-skel pfw-skel-line w40" />
                  <span className="pfw-skel pfw-skel-line w72" />
                </div>
              </div>
            </>
          )}
        ...
        {/* 会员横幅 + 资产卡 —— 仅自己的主页叠加；他人主页不渲染 */}
        {isMe && user && <MembershipBanner web user={user} onOpen={() => nav('/vip')} />}
        {isMe && user && <AssetCard web user={user} onWallet={() => nav('/wallet')} />}
        ...
        <ProfileContentTabs
          ...
          onRetry={load}
          label={isMe ? '我的主页内容' : '主页内容'}
        >
          {data && tab === 'characters' && (data.characters.length === 0 ? <div className="empty" style={{ padding: 40 }}>暂无公开角色</div> : (
            <div className="grid">
              {data.characters.map(c => (
                <div key={c.id} className="char-card" onClick={() => nav('/character/' + c.id)}>
                  <div className="cover">{c.avatar ? <img src={assetUrl(c.avatar)} alt="" loading="lazy" /> : <div className="ph cover-art-box"><CoverArt name={c.name} /></div>}</div>
          ...
          {data && tab === 'scripts' && (data.scripts.length === 0 ? <div className="empty" style={{ padding: 40 }}>暂无剧本</div> : (
            <div className="grid">
              {data.scripts.map(s => (
                <div key={s.id} className="char-card" onClick={() => nav('/script/' + s.id)}>
                  <div className="cover">{s.cover ? <img src={assetUrl(s.cover)} alt="" loading="lazy" /> : <div className="ph"><ScrollText size={32} /></div>}
          ...
          {data && tab === 'moments' && (data.moments.length === 0 ? <div className="empty" style={{ padding: 40 }}>暂无动态</div> : (
            data.moments.map(m => (
              <div key={m.id} className="card" style={{ marginBottom: 12 }}>
                <div style={{ whiteSpace: 'pre-wrap', lineHeight: 1.7 }}>{m.text}</div>
                {m.image && <img src={assetUrl(m.image)} style={{ marginTop: 10, borderRadius: 12, maxHeight: 240, maxWidth: '100%' }} alt="" />}
