...
import { Crown, Coins, Gem, Settings, ScrollText, UserPlus, UserCheck, LogOut, Wallet } from 'lucide-react';
        ...
        {tab === 'moments' && (data.moments.length === 0 ? <div className="empty" style={{ padding: 40 }}>暂无动态</div> : (
          data.moments.map(m => (
            <div key={m.id} className="card" style={{ marginBottom: 12 }}>
              ...
              <div className="muted" style={{ fontSize: 12, marginTop: 8 }}>♥ {m.likes} · {m.created_at}</div>
            </div>
          ))
        ))}
      </div>
