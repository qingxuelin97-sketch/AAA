...
// gentle → calm 下发；neutral 不下发（让 MiniMax 按文本自动情绪化）。
...
// VIP 档位（与服务端 economy.js 一致）：周卡 / 月卡 / 季卡；缺省 month。
const VIP_PLANS = {
  week:   { id: 'week',   label: '周卡', days: 7,  gold: 8000 },
  month:  { id: 'month',  label: '月卡', days: 30, gold: 30000 },
  season: { id: 'season', label: '季卡', days: 90, gold: 78000 }
};
const isVip = (u) => !!u?.vip_until && new Date(u.vip_until).getTime() > Date.now();
function publicUser(u) {
  return u && { id: u.id, username: u.username, email: u.email, display_name: u.display_name, avatar: u.avatar, banner: u.banner, bio: u.bio, gold: u.gold, diamond: u.diamond, chat_credits: u.chat_credits || 0, vip_until: u.vip_until, vip: isVip(u), checkin_streak: u.checkin_streak, last_checkin: u.last_checkin, is_gm: !!u.is_gm, is_banned: !!u.is_banned, svip: !!u.svip, verified: !!u.verified, verified_note: u.verified_note || '', is_councilor: !!u.is_councilor, official: !!u.official, creator_tier: creatorTier(u), created_at: u.created_at, avatar_frame: u.avatar_frame || '' };
}
...
async function route(method, path, search, body, headers) {
  ...
  // ---------- chat ----------
  if (method === 'GET' && path === '/chat/conversations') { need(); const rows = filter('conversations', c => c.user_id === me.id).map(c => { const ch = find('characters', x => x.id === c.character_id); const msgs = filter('messages', x => x.conversation_id === c.id); return { ...c, character_name: ch?.name, character_avatar: ch?.avatar, last_message: msgs.length ? msgs[msgs.length - 1].content : '' }; }).sort((a, b) => ((b.pinned || 0) - (a.pinned || 0)) || (b.updated_at || '').localeCompare(a.updated_at || '')); return J({ conversations: rows }); }
  ...
  if ((m = P(/^\/chat\/conversations\/(\d+)$/))) {
    ...
    if (method === 'PATCH') {
      ...
      if (body.muted !== undefined) conv.muted = body.muted ? 1 : 0;
      const markOnly = (body.pinned !== undefined || body.muted !== undefined)
        && !(typeof body.title === 'string' && body.title.trim()) && !body.clear;
      if (!markOnly) conv.updated_at = now();
      save();
