...
- **阿里云 / 任意 Ubuntu 服务器（数据存本机硬盘，永不丢）**：
  ```bash
  apt-get update -y && apt-get install -y git
  git clone https://github.com/qingxuelin97-sketch/AAA.git /opt/huanyu
  cd /opt/huanyu && bash deploy/install.sh      # 一键安装 + 启动（端口 4000）
  ```
  之后更新到最新代码：`cd /opt/huanyu && bash deploy/update.sh`。
  （记得在安全组放行 TCP 4000；脚本已内置 swap、Node 22 自动安装/升级、Vite 原生二进制修复、pm2 常驻。）

  **部署报错排查**：
  - `npm error E404 …cdn.npmmirror.com/packages/@fontsource/noto-serif-sc/…tgz`：npmmirror 镜像漏同步了个别版本的 tarball。仓库根目录 `.npmrc` 已把 `@fontsource*` / `@rolldown` 固定到官方源规避；老服务器只需 `git pull` 后重跑 `bash deploy/update.sh`（脚本还带整体回退官方源的重试）。也可顺手触发镜像补同步：打开 `https://npmmirror.com/sync/@fontsource/noto-serif-sc`。
  - `npm warn EBADENGINE`（vite@8 / rolldown 要求 `^20.19.0 || >=22.12.0`，服务器是 v20.18.x）：`deploy/ensure-node.sh` 会在 install/update 时自动升级到 Node 22 LTS（装到 `/opt/node22` 并接管软链），无需手动处理。
...
- **微信云托管 / 任意 MySQL**：在控制台「**关联 MySQL**」后平台会注入 `MYSQL_*` 环境变量，服务端**自动识别、自动建库建表**，无需手填；也可手动设 `BACKUP_MYSQL_URL=mysql://用户:密码@主机:3306/库名`。
