...
## 日志板块「全链路观测」大更新（2026-08-05）

日志系统（服务端 logger / 三端客户端采集 / GM 日志台）一次性修复全部已知
缺陷并大幅扩充能力。
...
- **链路下钻**：查询支持 request_id / session_id / fingerprint 精确过滤与
  ...
  「查看同请求链路 / 同会话 / 同类错误」，链路视图自动按时间正序。
- **阈值告警**：窗口期内 error/fatal 累计达到阈值 → 站内通知全体 GM +
  SSE 弹条，带冷却期防告警风暴；规则（启用/阈值/窗口/冷却）GM 后台可调。
- **保留策略可配置**：各级别保留天数（默认 debug 3d / info 7d / warn 30d /
  error+fatal 90d）GM 可调（1–365 钳制）；清理支持按级别 + 「早于 N 天」
  定向执行。
- **统计与图表增强**：24h 请求量 / 错误率 / 慢请求（计数·均值·最大耗时）/
  状态码分档分布；趋势图 24 小时 / 30 天切换 + 仅错误视图，时序零填充
  不再「缺牙」；TOP 榜新增类别 / 状态码 / 慢接口（按均值耗时）维度。
- **导出三格式**：JSON / CSV（带 BOM，Excel 直开）/ NDJSON，上限 5000 条，
  导出行为进审计。
- **审计覆盖补齐**：平台 AI 配置、SMTP 配置、注册白名单、兑换码、内容
  管控（角色/剧本/动态/评论/评价删除）等 GM 敏感操作全部留痕（密钥类
  只记字段名不记值）。
- **GM 日志台**：时间范围预设（1h/24h/7d/自定义本地时间，自动折算 UTC）、
...
- 新命令 `npm run test:logs`（`server/logs-test.mjs`）：批量摄入 / level
  钳制 / 计数透传 / 链路过滤 / 统计时序 / 保留钳制 / 阈值告警 / 导出格式 /
  定向清理 / 审计留痕十组验值。
- `logs` 表补 endpoint / session_id 索引。
